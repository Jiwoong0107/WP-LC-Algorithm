    ...
    while (!Q.empty()) {
        ...
        for (const auto& edge : adj[u]) {
            ...
            for (int l_idx : labels_to_propagate) {
                ...
                if (is_pruned(v, next_c)) { pruned_count++; continue; }

                if (is_danger[v] || is_non_positive_edge) {
                    if (check_and_record_cycle(l_idx, v, edge)) continue;
                }

                int new_idx = update_label_set_and_create(v, next_c, l_idx);
                if (new_idx != -1) {
...
int DiagnosticBiObjectiveBellmanFord::update_label_set_and_create(int v, const Cost& new_cost, int p_idx) {
...
bool DiagnosticBiObjectiveBellmanFord::check_and_record_cycle(int end_idx, int v, const Edge& edge) {
    int cur = end_idx;
    bool found = false;
